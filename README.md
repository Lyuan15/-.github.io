...
- 同一个设备的某一功能，在固定的账号（任意house下）失效。为什么设备的功能会和账号有关系？

  经过研发排查，关键问题在于日志中mqtt承载的payload内容错误。未解决。

  gpt：云端后台排查。检查该账号在云端推送订阅表（如MQTT topic分组或者FCM/APNS推送策略），是否和其它账号有区别；查询该账号下所有事件发出的raw payload和服务器log，有无报错、断点、状态码等异常。
  但如果问题出在服务器端的推送权限，之前同样的账号和house下，设备功能完全正常？？
...
- 后面还在做的事（比较耗时间的）：
 ...
 - webrtc vs. p2p
  ...
  webrtc: DTLS/SRTP/SCTP，内置端到端加密

  P2P: 点对点拉取文件
  
  webrtc: 更专注于实时音视频通信与数据通道

  P2P:
  
    - 发现伙伴 得知A和B的地址，如IP、端口。
    - 发起连接 TCP/UDP
    - 传输数据
    - 网络穿透 这个需要手动处理，非P2P协议自动执行。大多数情况下，A和B都在路由下而非公网IP，需要中介服务器帮忙知到具体的位置，然后利用NAT打洞技术强行建立连接。这样依旧是属于AB之间直接传输文件而非中介服务器，所以还是属于P2P。
  
  webrtc:
  
    - 开始之前的信令 不属于webtrc，是要自己选择一个“如何实现连接”的协议，一般采用websocket或http。利用该信令，屏和cam先用信令服务器交换参数，如IP、端口、会话描述等。
    - NAT穿透 屏和cam先用ICE协议+STUN服务器测试自己在网络的外部地址和端口，尝试能不能用打洞原理和对方直连。如果是较简单的方式，如处于同一局域网则马上直连成功；如果是较复杂的情况则需要TURN服务器做中继帮忙连接。
    - 建立连接传输数据 原理还是基于P2P，点对点的传输音视频流和数据流，但webrtc会额外自动处理码流和加密。屏和cam也是直接传输，除非是穿透建立连接时只能使用TURN服务器。两端的实时沟通可以随时断开，自动回收资源。
  
  
  webrtc基于P2P基础上，加了：
  
  - 自动穿透
  - 自动处理码流+网络拥塞控制机制 带宽自适应，丢包重传，拥塞控制等机制。这里虽然P2P使用的TCP也有这些机制，但TCP不适合实时性很高的音视频流传输。webrtc使用的核心是RTP，仅对关键数据做补救，不会等太久，所以WebRTC能在极端网络环境下保持“不断流”的体验。
  - 自动加密 自动采用SRTP协议，全链路加密。P2P可能使用明文。
  - 防抖 自行维护网络状态，抗抖动缓冲、丢包恢复、音频回声消除、自动纠错。
  - 编解码 适用于音视频的编解码功能，兼容各类终端和系统

  mqtt偶然漏发消息？
  
  在扫码登陆功能下，ios扫码后智慧屏二维码请求成功，MQTT注册成功，用户扫码成功，智慧屏收到“扫码成功”推送（1），但是用户在手机上点击登陆以后，智慧屏并没有收到 “确认登录”推送（2）。是一个很难复现的bug。
  
  A: 有可能服务端在收到手机端“确认登录”事件后，消息未成功推送到MQTT Broker。也有可能推送到MQTT Broker后，由于网络或者MQTT Broker负载等，消息未下发到屏端。暂不清楚Qos等级。也有可能是“扫码成功”和“确认登陆”消息的推送顺序出了问题？
ps：有两次扫码登陆失败，但是屏端log只能查到其中一次，没拿到ioslog不清楚另一次是否问题出在ios端。
